...
| MASVS-AUTH-1 | Native OPAQUE handoff, generic errors, missing-account dummy path, one-time state, rate-limit contract, and passkey-first Web adapter exist. | partial | Verify deployed account policy, session issuance, WebAuthn challenge/origin/RP-ID validation. |
