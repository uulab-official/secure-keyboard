...
| MASVS-AUTH-1 | Native OPAQUE handoff, generic errors, missing-account dummy path, one-time state, rate-limit contract, passkey-first Web adapter, pinned WebAuthn reference service/HTTP contract, required host-validated `csrf_validated` request field, compile-tested Axum/Actix integrations, bounded Node/TypeScript Fetch transport bridge, and injectable WebAuthn storage contracts exist with host-principal binding. | partial | Verify the deployed backend implementation, account policy, actual CSRF/session validator, durable credential/ceremony records, and distributed replay controls. |
...
| MASVS-NETWORK-2 | Key IDs and downgrade windows are explicit in the protocol contract; v1 explicitly assigns TLS/pinning to the host-native transport and documents the required fail-closed pinning policy. | partial | Verify the deployed host pinning decision and rotation tests where the product selects pinning. |
...
| MASVS-CODE-1 | Rust toolchain, protocol suite, package versions, lockfiles, audit, fuzz, SBOM, native checksums, and machine-readable release evidence gates are pinned; `docs/PLATFORM-SUPPORT.json` and the physical-device verifier enforce OS/API and patch floors. | partial | Independently review the vendor patch artifacts and confirm the selected product support window remains current. |
| MASVS-RESILIENCE-1 | v1 documents that compromised/rooted runtimes are outside the trusted boundary and assigns attestation/tamper response to the product. | partial | Verify the product's selected deny/step-up/limit policy and device evidence, or record the accepted residual risk. |
