...
| MASVS-AUTH-1 | Native OPAQUE handoff, generic errors, missing-account dummy path, one-time state, rate-limit contract, passkey-first Web adapter, and pinned WebAuthn reference service/HTTP contract exist with host-principal binding. | partial | Verify deployed account policy, session issuance, durable WebAuthn credential/ceremony stores, framework HTTP integration, and distributed replay controls. |
| MASVS-NETWORK-1 | `secure-auth-http` enforces bounded JSON routes and documents TLS as mandatory; it does not implement TLS itself. | partial | Test endpoint identity, TLS versions, proxy limits, and cleartext rejection in each host app. |
...
| MASVS-PLATFORM-1 | C ABI uses opaque handles; Android ownership and FFI null/ownership tests are executable. | partial | Compile and inspect RN native manager and Flutter PlatformView bridges in real host apps. |
| MASVS-PLATFORM-3 | iOS background/capture masking, Android `FLAG_SECURE`, autofill exclusion, and accessibility contracts exist. | partial | Run screenshot, task-switcher, screen-recording, autofill, and accessibility tests on devices. |
| MASVS-CODE-1 | Rust toolchain, protocol suite, package versions, lockfiles, audit, fuzz, and SBOM gates are pinned. | partial | Define supported OS/API matrix and enforce minimum patched platform versions. |
