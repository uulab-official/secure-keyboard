...
| MASVS-AUTH-1 | Native OPAQUE handoff, generic errors, missing-account dummy path, one-time state, rate-limit contract, passkey-first Web adapter, pinned WebAuthn reference service/HTTP contract, and compile-tested Axum integration exist with host-principal binding. | partial | Verify deployed account policy, session/CSRF integration, durable WebAuthn credential/ceremony stores, and distributed replay controls. |
