...
| MASVS-AUTH-1 | Native OPAQUE handoff, generic errors, missing-account dummy path, one-time state, rate-limit contract, passkey-first Web adapter, pinned WebAuthn reference service/HTTP contract, required host-validated `csrf_validated` request field, compile-tested Axum/Actix integrations, and injectable WebAuthn storage contracts exist with host-principal binding. | partial | Verify the deployed backend implementation, account policy, actual CSRF/session validator, durable credential/ceremony records, and distributed replay controls. |
