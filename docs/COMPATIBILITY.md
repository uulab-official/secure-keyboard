...
| Actix Web server adapter | package `0.1.0`; `actix-web = 4.11.0` | Use the workspace lockfile, pass a validated deployment context, provide a request-parts CSRF validation callback, and keep TLS/proxy, rate-limit, session, and durable-store policy in the host application. |
