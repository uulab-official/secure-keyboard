...
| Axum server adapter | package `0.1.0`; `axum = 0.8.9`; optional `webauthn` feature | Use the workspace lockfile, pass a validated deployment context, provide request-parts CSRF validation callbacks, and for `financial_router` resolve a fresh `FinancialAuthContext` and return provider/subject/operation/nonce/deployment-bound evidence. Keep TLS/proxy, rate-limit, shared nonce, session, and durable-store policy in the host application. |
