...
| Axum server adapter | package `0.1.0`; `axum = 0.8.9`; optional `webauthn` feature | Use the workspace lockfile, pass a validated deployment context, resolve WebAuthn principals from host-session request parts only, and keep TLS/proxy, rate-limit, session, and durable-store policy in the host application. |
