...
| Node/TypeScript server adapter | package `0.1.0`; HTTP transport contract `1`; Node Web Fetch APIs; TypeScript `5.9.3` in the workspace lockfile | Use `@secure-keypad/server-node` only as a bounded transport bridge to the pinned Rust/native OPAQUE service. Its `NODE_SERVER_CONTRACT_VERSION` must match Rust `HTTP_CONTRACT_VERSION`; the parity test/check are release gates. Supply host-validated TLS/proxy facts and CSRF/origin validation; never treat JavaScript memory as a secure secret boundary or implement OPAQUE in application JavaScript. |
