...
The public controller is limited to operations such as `beginSession`, `pressKey(keyId)`, `backspace`, `clear`, `submit`, and `cancel`. The layout contract includes a `cancel` action role; activating it calls the native cancellation path, clears the core buffer, and emits only `displayState: cancelled` plus a `cancelled` result code. State events expose only length, masked display state, validation state, and result codes.
