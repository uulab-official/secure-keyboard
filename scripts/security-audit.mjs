...
export function runSecurityAudit() {
  ...
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /const upstreamBodyLimitBytes = isReady\(options\.deploymentContext\)[\s\S]{0,2200}contentType = request\.headers\.get\("content-type"\)[\s\S]{0,1200}catch\s*\{/, "Node server request metadata access must stay inside a fail-closed boundary");
