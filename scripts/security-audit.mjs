...
export function runSecurityAudit() {
  ...
  requireText(findings, "crates/secure-webauthn-example/src/storage_redis.rs", webauthnRedis, /CONSUME_SCRIPT[\s\S]{0,520}PTTL[\s\S]{0,220}ttl == -2[\s\S]{0,260}ttl < 1[\s\S]{0,220}tonumber\(ARGV\[2\]\)[\s\S]{0,220}STRLEN/, "Redis WebAuthn ceremony consumption must reject missing or drifted TTLs before materialization");
