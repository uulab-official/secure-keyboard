...
export function runSecurityAudit() {
  ...
  for (const file of [
    ...
    requireText(findings, file, contents, /try validate\(theme: theme\)[\s\S]{0,700}releaseNativeSessionPreservingConfiguration\(\)/, "iOS native reconfiguration must clear the old session through its zeroizing release path");
  ...
  requireText(findings, "crates/secure-ffi/src/lib.rs", ffiImplementation, /secure_keypad_client_login_finish[\s\S]*?auth_finish_arguments_alias\(AuthFinishArguments \{[\s\S]*?input_slot: login/, "FFI login finish must apply live-object alias validation on its own path");
  requireText(findings, "crates/secure-ffi/src/lib.rs", ffiImplementation, /secure_keypad_client_registration_finish[\s\S]*?auth_finish_arguments_alias\(AuthFinishArguments \{[\s\S]*?input_slot: registration/, "FFI registration finish must apply live-object alias validation on its own path");
  ...
  requireText(findings, "packages/web/src/index.ts", web, /function throwIfAborted\(signal: AbortSignal \| undefined\)[\s\S]{0,160}signal\?\.aborted/, "WebAuthn adapter must expose a stable abort gate");
  requireText(findings, "packages/web/src/index.ts", web, /environment\.credentials!\.create\([\s\S]{0,300}\);\s*throwIfAborted\(operationOptions\.signal\)/, "WebAuthn registration must discard late credentials before serialization after cancellation");
  requireText(findings, "packages/web/src/index.ts", web, /environment\.credentials!\.get\([\s\S]{0,300}\);\s*throwIfAborted\(operationOptions\.signal\)/, "WebAuthn authentication must discard late credentials before serialization after cancellation");
  requireText(findings, "packages/web/src/index.ts", web, /return serializeRegistrationCredential\(credential\);[\s\S]{0,180}\} catch \(error\) \{\s*throwIfAborted\(operationOptions\.signal\)/, "WebAuthn registration must preserve cancellation when the browser rejects after cancellation");
  requireText(findings, "packages/web/src/index.ts", web, /return serializeAssertionCredential\(credential\);[\s\S]{0,180}\} catch \(error\) \{\s*throwIfAborted\(operationOptions\.signal\)/, "WebAuthn authentication must preserve cancellation when the browser rejects after cancellation");
  ...
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /await csrfValidated\(request\)[\s\S]{0,2600}readBoundedBody/, "Node server adapter must validate CSRF before buffering the body");
  ...
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /NodeSecurityProfile\s*=\s*"standard"\s*\|\s*"financial"/, "Node server adapter must expose an explicit financial security profile");
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /financialContext\?:/, "Node financial profile must expose a bound financial-context resolver");
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /deviceIntegrityVerifier\?:/, "Node financial profile must expose a structured device-integrity verifier");
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /function isFreshBoundEvidence[\s\S]{0,1600}expiresAtMs - issuedAtMs <= 300_000/, "Node financial profile must enforce bounded attestation freshness");
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /function consumeFinancialEvidence[\s\S]{0,1000}MAX_FINANCIAL_REPLAY_ENTRIES/, "Node financial profile must bound local attestation replay state");
  ...
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /const upstreamBodyLimitBytes = isReady\(deploymentContext\)[\s\S]{0,4000}contentType = request\.headers\.get\("content-type"\)[\s\S]{0,2000}catch\s*\{/, "Node server request metadata access must stay inside a fail-closed boundary");
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /if \(rateLimitDecision === undefined\)/, "Node server adapter must fail closed when rate-limit admission is not configured");
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /await rateLimitDecision\(request\)[\s\S]{0,3600}readBoundedBody/, "Node server adapter must resolve rate-limit admission before buffering the body");
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /securityProfile === "financial"[\s\S]{0,1800}deviceIntegrityVerifier[\s\S]{0,1800}isFreshBoundEvidence[\s\S]{0,1000}readBoundedBody/, "Node financial security profile must verify bound device evidence before buffering the body");
  ...
  requireText(findings, "packages/server-node/src/index.ts", nodeServer, /function isBodyTooLargeError\(error: unknown\)[\s\S]{0,240}\btry\s*\{[\s\S]{0,240}\}\s*catch\s*\{/, "Node server adapter must tolerate hostile body-reader errors");
  ...
  requireText(findings, "crates/secure-webauthn-example/src/storage_redis.rs", webauthnRedis, /CONSUME_SCRIPT[\s\S]{0,520}PTTL[\s\S]{0,220}ttl == -2[\s\S]{0,260}ttl < 1[\s\S]{0,220}tonumber\(ARGV\[2\]\)[\s\S]{0,400}STRLEN/, "Redis WebAuthn ceremony consumption must reject missing or drifted TTLs before materialization");
  ...
  requireText(findings, "crates/secure-auth-server/src/opaque_state_postgres.rs", opaqueStatePostgres, /POSTGRES_ONE_TIME_STATE_CONSUME_SQL[\s\S]{0,700}WITH removed AS[\s\S]{0,240}DELETE FROM secure_keypad_opaque_login_states[\s\S]{0,240}RETURNING state, expires_at[\s\S]{0,240}SELECT CASE/, "PostgreSQL OPAQUE state consumption must be atomic");
  ...
  const threatModel = source("docs/THREAT-MODEL.md", findings);
  requireText(findings, "docs/THREAT-MODEL.md", threatModel, /## Assets/, "threat model must enumerate protected assets");
  requireText(findings, "docs/THREAT-MODEL.md", threatModel, /## Trust boundaries/, "threat model must define trust boundaries");
  requireText(findings, "docs/THREAT-MODEL.md", threatModel, /## Adversaries/, "threat model must define adversaries and out-of-scope compromises");
  requireText(findings, "docs/THREAT-MODEL.md", threatModel, /## Residual risk and operator duties/, "threat model must assign residual risk to operators");
  requireText(findings, "docs/THREAT-MODEL.md", threatModel, /independent-review signature/, "threat model must require independently signed release evidence");
  ...
  requireText(findings, "scripts/check-device-evidence.mjs", deviceEvidenceCheck, /validateTestEvidence/, "physical device evidence must bind test claims to declared evidence");
  ...
  requireText(findings, "scripts/emit-native-device-evidence.mjs", nativeEvidenceEmitter, /testEvidence/, "native evidence emitter must record test-to-evidence bindings");
  requireText(findings, "scripts/emit-native-device-evidence.mjs", nativeEvidenceEmitter, /--test-evidence/, "native evidence CLI must accept test-to-evidence bindings");
  ...
  requireText(
    ...
    "release candidates must move all generated Flutter state outside the checkout before provenance metadata",
  );
  ...
  const androidRuntimeSmokeScript = source("scripts/android-emulator-runtime-smoke.sh", findings);
  requireText(findings, "scripts/android-emulator-runtime-smoke.sh", androidRuntimeSmokeScript, /UI_DUMP_PATH/, "Android runtime smoke must retain its accessibility hierarchy dump");
  requireText(findings, "scripts/android-emulator-runtime-smoke.sh", androidRuntimeSmokeScript, /android\.widget\.EditText/, "Android runtime smoke must reject editable text controls");
  requireText(findings, "scripts/android-emulator-runtime-smoke.sh", androidRuntimeSmokeScript, /password="true"/, "Android runtime smoke must reject password accessibility nodes");
