...
once. Input/output buffers and output slots must not overlap live opaque handle
objects or other output slots; the ABI rejects those calls with
`SECURE_KEYPAD_INVALID_ARGUMENT`. Build and ship the library from the same
immutable source revision as the native renderer; do not treat the C ABI alone
as proof of device-level capture, accessibility, or compromised-runtime
protection.
