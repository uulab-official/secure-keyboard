...
- `secure-auth-http`: bounded framework-neutral HTTP/JSON route contract for OPAQUE registration/login; every call requires a validated TLS/proxy-limit deployment context and an explicit host-validated CSRF result, while rate limits, account enrollment, and session issuance remain host-server responsibilities. Its credential repository contract is atomic create-only, so registration cannot replace an existing credential.
...
- `secure-auth-actix`: compile-tested Actix Web adapter for the same OPAQUE HTTP contract; it validates CSRF from request parts before bounded payload buffering and preserves the generic error/security-header boundary. Its optional `webauthn` feature adds the passkey route with a body-free host-principal resolver.
- `secure-webauthn-example`: Rust 1.88-compatible, `webauthn-rs 0.5.4`-pinned passkey registration/authentication service with origin/RP-ID binding, bounded HTTP/JSON routes, host-principal binding, authenticated encrypted ceremony state, atomic one-time consume, and injectable credential/ceremony storage contracts.
